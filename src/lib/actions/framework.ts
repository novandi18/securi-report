...
import { cweEntries, owaspEntries } from "@/lib/db/schema";
import { cweCreateSchema, owaspCreateSchema } from "@/lib/validations/kb";
...
export async function getCweEntriesAction() {
  ...
  try {
    const data = await db.select().from(cweEntries).orderBy(cweEntries.id);
    return { success: true as const, data };
  } catch (error) {
    console.error("Failed to fetch CWE entries:", error);
    return {
      success: false as const,
      error: "Failed to fetch CWE entries",
      data: [],
    };
  }
}

export async function createCweEntryAction(
  ...
  if (!session || session.user.role !== "administrator") {
    return {
      success: false,
      error: "Only administrators can manage CWE entries.",
    };
  }
  ...
  const parsed = cweCreateSchema.safeParse(raw);
  if (!parsed.success) {
    return {
      success: false,
  ...
  try {
    // Check for duplicate CWE ID
    const [existing] = await db
      .select()
      .from(cweEntries)
      .where(eq(cweEntries.id, parsed.data.id))
      .limit(1);

    if (existing) {
      return {
        success: false,
        error: `CWE-${parsed.data.id} already exists.`,
        values: raw,
      };
    }

    await db.insert(cweEntries).values({
      id: parsed.data.id,
      title: parsed.data.title,
      description: parsed.data.description ?? null,
    });

    await audit({ userId: session.user.id, action: "cwe.create", resourceId: String(parsed.data.id) });
    revalidatePath("/kb/frameworks");
    return { success: true };
  } catch (error) {
    console.error("Failed to create CWE entry:", error);
    return { success: false, error: "Failed to create CWE entry.", values: raw };
  }
}

export async function deleteCweEntryAction(id: number): Promise<ActionResult> {
  const session = await auth();
  if (!session || session.user.role !== "administrator") {
    return {
      success: false,
      error: "Only administrators can delete CWE entries.",
    };
  }
  ...
  try {
    await db.delete(cweEntries).where(eq(cweEntries.id, id));
    await audit({ userId: session.user.id, action: "cwe.delete", resourceId: String(id) });
    revalidatePath("/kb/frameworks");
    return { success: true };
  } catch (error) {
    console.error("Failed to delete CWE entry:", error);
    return { success: false, error: "Failed to delete CWE entry." };
  }
}
...
export async function getOwaspEntriesAction() {
  const session = await auth();
  if (!session) {
    return { success: false as const, error: "Unauthorized", data: [] };
  }
  ...
  try {
    ...
    console.error("Failed to fetch OWASP entries:", error);
    return {
      success: false as const,
      error: "Failed to fetch OWASP entries",
      data: [],
    };
  }
}

export async function createOwaspEntryAction(
  _prevState: ActionResult | null,
  formData: FormData,
): Promise<ActionResult> {
  const session = await auth();
  if (!session || session.user.role !== "administrator") {
    return {
      success: false,
      error: "Only administrators can manage OWASP entries.",
    };
  }
  ...
  const parsed = owaspCreateSchema.safeParse(raw);
  if (!parsed.success) {
    return {
      success: false,
      error: "Validation failed. Please check your input.",
  ...
  try {
    await db.insert(owaspEntries).values({
      code: parsed.data.code,
      title: parsed.data.title,
      version: parsed.data.version,
    });

    await audit({ userId: session.user.id, action: "owasp.create" });
    revalidatePath("/kb/frameworks");
    return { success: true };
  } catch (error) {
    console.error("Failed to create OWASP entry:", error);
    return {
      success: false,
      error: "Failed to create OWASP entry.",
      values: raw,
    };
  }
}

export async function deleteOwaspEntryAction(
  id: number,
): Promise<ActionResult> {
  const session = await auth();
  if (!session || session.user.role !== "administrator") {
    return {
      success: false,
      error: "Only administrators can delete OWASP entries.",
    };
  }
  ...
  try {
    await db.delete(owaspEntries).where(eq(owaspEntries.id, id));
    await audit({ userId: session.user.id, action: "owasp.delete", resourceId: String(id) });
    revalidatePath("/kb/frameworks");
    return { success: true };
  } catch (error) {
    console.error("Failed to delete OWASP entry:", error);
    return { success: false, error: "Failed to delete OWASP entry." };
  }
}

// ═══════════════════════════════════════════════════════════
//  SEED / IMPORT CWE & OWASP DATA
// ═══════════════════════════════════════════════════════════
...
export async function seedFrameworksAction(): Promise<ActionResult> {
  const session = await auth();
  if (!session || session.user.role !== "administrator") {
    return {
      success: false,
      error: "Only administrators can seed framework data.",
    };
  }

  try {
    // Seed CWE entries (ignore duplicates)
    ...
    // Seed OWASP 2021
    for (const entry of OWASP_2021_SEED_DATA) {
      const existing = await db
        .select()
        .from(owaspEntries)
    ...
    // Seed OWASP 2025
    for (const entry of OWASP_2025_SEED_DATA) {
      const existing = await db
        .select()
        .from(owaspEntries)
        .where(eq(owaspEntries.code, entry.code))
    ...
    console.error("Failed to seed framework data:", error);
    return { success: false, error: "Failed to seed framework data." };
  }
}
